...
IMPLEMENT_DYNAMIC(CPreFix1TableSet, CRecordset)

CPreFix1TableSet::CPreFix1TableSet(CDatabase* pdb)
	: CRecordset(pdb)
{
	//{{AFX_FIELD_INIT(CPreFix1TableSet)
	m_Num = 0;
	m_strCode = _T("");
	m_strName = _T("");
	m_Group = 0;
...
CString CPreFix1TableSet::GetDefaultConnect()
{
	return _T("ODBC;DSN=Repent;UID=Repent;PWD=Repent");
}

CString CPreFix1TableSet::GetDefaultSQL()
{
	return _T("[dbo].[PREFIX1]");
}

void CPreFix1TableSet::DoFieldExchange(CFieldExchange* pFX)
{
	//{{AFX_FIELD_MAP(CPreFix1TableSet)
...
/////////////////////////////////////////////////////////////////////////////
// CPreFix1TableSet diagnostics

#ifdef _DEBUG
void CPreFix1TableSet::AssertValid() const
{
	CRecordset::AssertValid();
}

void CPreFix1TableSet::Dump(CDumpContext& dc) const
{
	CRecordset::Dump(dc);
}
#endif //_DEBUG
