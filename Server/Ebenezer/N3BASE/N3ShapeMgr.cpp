...
bool CN3ShapeMgr::Save(HANDLE hFile)
{
	...
	for (int i = 0; i < iSC; i++)
	{
		...
		if (m_Shapes[i]->m_iEventID != 0
			|| m_Shapes[i]->m_iEventType != 0
			|| m_Shapes[i]->m_iNPC_ID != 0
			|| m_Shapes[i]->m_iNPC_Status != 0)
...
bool CN3ShapeMgr::SaveCollisionData(HANDLE hFile)
{
	...
	for (float fZ = 0.0f; fZ < m_fMapLength; fZ += CELL_MAIN_SIZE, z++)
	{
		...
		for (float fX = 0.0f; fX < m_fMapWidth; fX += CELL_MAIN_SIZE, x++)
		{
			...
			WriteFile(hFile, &bExist, 4, &dwRWC, nullptr); // 데이터가 있는 셀인지 쓰고..
	...
	for (int zM = zMainS; zM < zMainE; zM++)
	{
		for (int xM = xMainS; xM < xMainE; xM++)
		{
			...
			for (i = 0; i < iSCC; i++)
			{
				...
				if (m_Shapes[iSIndex]->m_bDontRender)
					continue;
	...
	__Vector3* pVec)			// 충돌한 면 의 폴리곤 __Vector3[3]
{
	...
	int nSubCellCount = SubCellPathThru(vPos, vPosNext, ppCells); // 통과하는 서브셀을 가져온다..
	...
	{
		pShape = *it;
		if (nullptr == pShape->CollisionMesh())
			continue;
	...
	for (i = 0; i < iSC; i++)
	{
		...
		if (pVMesh->CheckCollision(Shapes[i]->m_Matrix, vPos, vPosNext, pvCol, pvNormal))
			return true;
	}
#endif // end of #ifndef _3DSERVER
	return false;
...
// 벡터 사이에 걸친 셀포인터 돌려준다..
int CN3ShapeMgr::SubCellPathThru(const __Vector3& vFrom, const __Vector3& vAt, __CellSub** ppSubCells)
{
	if (nullptr == ppSubCells)
		return 0;
